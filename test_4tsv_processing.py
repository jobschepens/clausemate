#!/usr/bin/env python3
"""
Test script for 4.tsv processing with incomplete format parser.

This script tests the new incomplete format parser to see if 4.tsv can now be processed
with graceful degradation for missing features.
"""
    ...
    try:
        ...
        for sentence_context in parser.parse_sentence_streaming(file_path):
            ...
            for i, token in enumerate(sentence_context.tokens[:3]):
                print(f"  Token {i+1}: '{token.text}' (idx={token.idx}, sentence={token.sentence_num})")
...
def test_main_analyzer():
    ...
    try:
        ...
        if relationships:
            ...
            print("No relationships found - this may be expected for incomplete format")
...
def main():
    ...
    if format_info:
        ...
        if format_info.format_type == "incomplete" and format_info.compatibility_score >= 0.5:
