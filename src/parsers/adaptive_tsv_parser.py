...
from src.parsers.preamble_parser import PreambleParser, extract_preamble_from_file, AnnotationSchema
from src.utils.format_detector import TSVFormatDetector, TSVFormatInfo, ColumnMapping
...
class AdaptiveTSVParser(BaseParser):
    ...
    def _setup_column_mapping(self):
        ...
        try:
            ...
            if file_path:
                ...
                self.current_annotation_schema = self.preamble_parser.parse_preamble_lines(preamble_lines)
                
                # Create dynamic column mapping based on preamble
                self.current_column_mapping = self._create_dynamic_column_mapping()
                
                logger.info(f"Using preamble-based column mapping:")
                logger.info(f"  Total columns: {self.current_annotation_schema.total_columns}")
                logger.info(f"  Coreference link: {self.current_column_mapping.coreference_link}")
                logger.info(f"  Coreference type: {self.current_column_mapping.coreference_type}")
            ...
            logger.warning(f"Preamble-based parsing failed, falling back to standard mapping: {e}")
        
        # Fallback to standard column mapping
        self.current_column_mapping = ColumnMapping()
        
        # Log the mapping being used
        logger.debug(f"Using standard column mapping for {self.current_format_info.format_type} format:")
        ...
        logger.debug(f"  Coreference Link: {self.current_column_mapping.coreference_link}")
        logger.debug(f"  Coreference Type: {self.current_column_mapping.coreference_type}")
            ...
            with open(file_path, encoding="utf-8") as file:
                ...
                for line_num, row in enumerate(reader, 1):
                    try:
                        ...
                        min_required_columns = 3  # Absolute minimum for basic token parsing
                        ...
                        if self.current_format_info:
                            if self.current_format_info.format_type == "incomplete":
                                ...
                                min_required_columns = max(
                                    self.current_column_mapping.coreference_type + 1 if self.current_column_mapping else 10,
                                    min_required_columns
                                )
                            else:
                                # For complete formats, use standard requirements
                                min_required_columns = max(
                                    self.current_column_mapping.inanimate_coreference_type + 1 if self.current_column_mapping else 14,
                                    min_required_columns
                                )
                        ...
                        if len(row) < min_required_columns:
                            ...
                            if not (self.current_format_info and
                                   self.current_format_info.format_type == "incomplete" and
                                   len(row) >= 12):  # 12+ columns for incomplete is acceptable
                                logger.warning(
    ...
    def parse_token_line_adaptive(self, line: str, line_num: int) -> Token:
        ...
        try:
            ...
            if not token_id_str or not text:
                raise ParseError(f"Missing required token ID or text")
            ...
            grammatical_role = self._safe_get_column(parts, mapping.grammatical_role, "")
            thematic_role = self._safe_get_column(parts, mapping.thematic_role, "")
            
            # Extract coreference information
            coreference_link = self._safe_get_column(parts, mapping.coreference_link, "_")
            coreference_type = self._safe_get_column(parts, mapping.coreference_type, "_")
            inanimate_coreference_link = self._safe_get_column(parts, mapping.inanimate_coreference_link, "_")
            inanimate_coreference_type = self._safe_get_column(parts, mapping.inanimate_coreference_type, "_")
            ...
            inanimate_coreference_link = None if inanimate_coreference_link == "_" else inanimate_coreference_link
            inanimate_coreference_type = None if inanimate_coreference_type == "_" else inanimate_coreference_type
            ...
            raise ParseError(f"Invalid token line format at line {line_num}: {line}. Error: {str(e)}")
    ...
    def _safe_get_column(self, parts: List[str], column_index: int, default: str = "") -> str:
