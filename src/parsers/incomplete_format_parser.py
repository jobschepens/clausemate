...
class IncompleteFormatParser(AdaptiveTSVParser):
    ...
    def __init__(self, processor: BaseTokenProcessor, limitations: Optional[list] = None):
    ...
    def _setup_column_mapping_for_incomplete(self, file_path: str) -> None:
        ...
        try:
            ...
            if preamble_lines:
                self.logger.info("Attempting preamble-based column mapping for incomplete format")
            ...
            self.logger.warning(f"Column mapping setup failed: {e}, using minimal mapping")
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for line in f:
                    ...
                    self.logger.info(f"Detected {column_count} columns in incomplete format")
    ...
    def _detect_available_features_from_schema(self) -> None:
        ...
        if self.current_column_mapping:
            if (hasattr(self.current_column_mapping, 'coreference_link') and
                hasattr(self.current_column_mapping, 'coreference_type')):
                self.available_features['coreference'] = True
    ...
    def _parse_incomplete_format_streaming(self, file_path: str) -> Iterator[SentenceContext]:
            ...
            with open(file_path, encoding="utf-8") as file:
                ...
                for line_num, row in enumerate(reader, 1):
                    try:
                        ...
                        token = self._create_token_from_row(row, current_sentence_id or "1")
    ...
    def _create_token_from_row(self, row: list, sentence_id: str) -> Optional[Token]:
        ...
        try:
            ...
            if len(row) < 3:
                self.logger.warning(f"Row too short: {len(row)} columns, need at least 3")
                return None
            ...
            lemma = None
            if self.column_mapping.get('lemma') and len(row) > self.column_mapping['lemma']:
                lemma = row[self.column_mapping['lemma']]
                if lemma == '_':
                    lemma = None
            
            pos_tag = None
            if self.column_mapping.get('pos_tag') and len(row) > self.column_mapping['pos_tag']:
