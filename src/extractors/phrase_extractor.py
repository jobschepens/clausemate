...
class PhraseExtractor(BasePhraseExtractor):
    ...
    def can_extract(self, context: SentenceContext) -> bool:
        ...
        return any(
            (token.coreference_link and token.coreference_link != '_') or
            (token.coreference_type and token.coreference_type != '_') or
            (token.inanimate_coreference_link and token.inanimate_coreference_link != '_') or
    ...
    def _get_coreference_ids(self, token: Token) -> List[str]:
        ...
        if not ids:
            ...
            if token.inanimate_coreference_type and token.inanimate_coreference_type != '_':
