...
class AdvancedAnalysisEngine:
    ...
    def analyze_character_tracking(
        ...
        for rel in relationships:
            ...
            chapter_meta = next((meta for meta in chapter_metadata if meta.chapter_number == chapter_num), None)
        ...
        for char_id, mentions in character_mentions.items():
            ...
            profile = self._build_character_profile(char_id, mentions, cross_chapter_connections)
            character_profiles[char_id] = profile
        
        self.logger.info(f"Character tracking complete: {len(character_profiles)} characters identified")
    ...
    def analyze_narrative_flow(
        ...
        self.logger.info(f"Narrative flow analysis complete: {len(narrative_segments)} segments identified")
    ...
    def analyze_cross_chapter_transitions(
        ...
        for i in range(len(chapter_metadata) - 1):
            ...
            transition = self._analyze_chapter_transition(
                current_chapter, next_chapter, character_profiles, cross_chapter_connections
            )
            transitions.append(transition)
        
        self.logger.info(f"Cross-chapter transition analysis complete: {len(transitions)} transitions analyzed")
        return transitions
    ...
    def calculate_performance_metrics(
        ...
        for meta in chapter_metadata:
            ...
            chapter_ratio = meta.total_relationships / total_relationships if total_relationships > 0 else 0
        ...
        chain_resolution_completeness = 0.90  # Estimated based on cross-chapter connections
        ...
        metrics = PerformanceMetrics(
            ...
            cross_chapter_resolution_time=total_time * 0.2,  # Estimated 20% of total time
    ...
    def create_comprehensive_analysis_report(
        ...
        report = {
            ...
            "character_analysis": {
                "character_profiles": {
                    char_id: asdict(profile) for char_id, profile in character_profiles.items()
                },
                "character_statistics": self._calculate_character_statistics(character_profiles)
            },
            "narrative_analysis": {
                "narrative_segments": [asdict(segment) for segment in narrative_segments],
                "narrative_statistics": self._calculate_narrative_statistics(narrative_segments)
            },
            "transition_analysis": {
                "chapter_transitions": [asdict(transition) for transition in transitions],
                "transition_statistics": self._calculate_transition_statistics(transitions)
            },
    ...
    def _extract_character_mentions(
        ...
        global_sentence_id = getattr(relationship, 'global_sentence_id', relationship.sentence_id)
        
        # Calculate sentence position within chapter
        sentence_position = 0.5  # Default middle
        if chapter_meta and chapter_meta.sentence_range[1] > chapter_meta.sentence_range[0]:
            position_in_chapter = (relationship.sentence_num - chapter_meta.sentence_range[0]) / (chapter_meta.sentence_range[1] - chapter_meta.sentence_range[0])
            sentence_position = max(0.0, min(1.0, position_in_chapter))
        
        # Extract pronoun mention
        if hasattr(relationship, 'pronoun_coref_ids') and relationship.pronoun_coref_ids:
            for chain_id in relationship.pronoun_coref_ids:
                mention = CharacterMention(
                    ...
                    narrative_importance=self._calculate_mention_importance(relationship, "pronoun")
                )
                character_mentions[str(chain_id)].append(mention)
        
        # Extract clause mate mention if it has coreference
        if hasattr(relationship.clause_mate, 'coreference_id') and relationship.clause_mate.coreference_id:
            ...
            mention = CharacterMention(
                ...
                sentence_position=sentence_position,
                narrative_importance=self._calculate_mention_importance(relationship, "clause_mate")
            )
            character_mentions[str(chain_id)].append(mention)
    
    def _calculate_mention_importance(self, relationship: ClauseMateRelationship, mention_type: str) -> float:
        ...
        if hasattr(relationship.clause_mate, 'animacy') and relationship.clause_mate.animacy.value == "anim":
            importance += 0.2
    ...
    def _build_character_profile(
        ...
        chapters_present = sorted(list(set(m.chapter_number for m in mentions)))
        ...
        alternative_names = [text for text, count in text_counts.items() if text != primary_name]
        
        # Calculate analysis metrics
        narrative_prominence = min(1.0, len(mentions) / 50.0)  # Normalize by expected max mentions
        character_consistency = self._calculate_character_consistency(mentions)
        cross_chapter_continuity = self._calculate_cross_chapter_continuity(char_id, cross_chapter_connections)
    ...
    def _calculate_character_consistency(self, mentions: List[CharacterMention]) -> float:
        ...
        importance_variance = sum((imp - sum(importances)/len(importances))**2 for imp in importances) / len(importances)
        
        # Combine metrics (lower variance = higher consistency)
        consistency = 1.0 - min(1.0, role_variety + importance_variance)
        return max(0.0, consistency)
    
    def _calculate_cross_chapter_continuity(self, char_id: str, cross_chapter_connections: List[CrossChapterConnection]) -> float:
        """Calculate cross-chapter continuity score."""
        # Check if character appears in cross-chapter connections
        char_connections = [conn for conn in cross_chapter_connections if char_id in conn.chain_id]
        
        if not char_connections:
            return 0.0
        
        # Calculate continuity based on connection strength
        avg_strength = sum(conn.strength for conn in char_connections) / len(char_connections)
        return avg_strength
    ...
    def _calculate_dialogue_frequency(self, mentions: List[CharacterMention]) -> float:
        ...
        dialogue_indicators = sum(1 for m in mentions if "said" in m.mention_text.lower() or '"' in m.mention_text)
    ...
    def _segment_chapter_narrative(
        ...
        for i in range(4):
            ...
            segment_rels = [
                rel for rel in chapter_relationships
                if start_sentence <= rel.sentence_num <= end_sentence
            ]
            
            # Calculate segment metrics
            character_density = len(set(getattr(rel, 'pronoun_coref_ids', []) for rel in segment_rels)) / len(segment_rels) if segment_rels else 0
            coreference_density = sum(len(getattr(rel, 'pronoun_coref_ids', [])) for rel in segment_rels) / len(segment_rels) if segment_rels else 0
    ...
    def _analyze_chapter_transition(
        ...
        character_continuity = len(shared_characters) / len(current_chars) if current_chars else 0.0
        
        # Find connections between these chapters
        chapter_connections = [
            conn for conn in cross_chapter_connections
            if conn.from_chapter == current_chapter.chapter_number and conn.to_chapter == next_chapter.chapter_number
        ]
        
        thematic_continuity = sum(conn.strength for conn in chapter_connections) / len(chapter_connections) if chapter_connections else 0.0
    ...
    def _calculate_character_statistics(self, character_profiles: Dict[str, CharacterProfile]) -> Dict[str, Any]:
        ...
        return {
            ...
            "cross_chapter_characters": sum(1 for p in character_profiles.values() if len(p.chapters_present) > 1)
        }
    
    def _calculate_narrative_statistics(self, narrative_segments: List[NarrativeFlowSegment]) -> Dict[str, Any]:
        ...
        return {
            ...
            "climax_segments": sum(1 for s in narrative_segments if s.segment_type == "climax"),
            "development_segments": sum(1 for s in narrative_segments if s.segment_type == "development")
        }
    
    def _calculate_transition_statistics(self, transitions: List[CrossChapterTransition]) -> Dict[str, Any]:
    ...
    def _generate_analysis_recommendations(
        ...
        major_chars = sum(1 for p in character_profiles.values() if p.narrative_prominence > 0.8)
        if major_chars < 3:
            recommendations.append("Consider developing more major characters for richer narrative complexity")
        
        # Narrative flow recommendations
        avg_tension = sum(s.narrative_tension for s in narrative_segments) / len(narrative_segments) if narrative_segments else 0
        if avg_tension < 0.5:
            recommendations.append("Consider increasing narrative tension in development segments")
        
        # Transition recommendations
        weak_transitions = sum(1 for t in transitions if t.narrative_coherence < 0.3)
        if weak_transitions > len(transitions) / 2:
            recommendations.append("Consider strengthening character continuity between chapters")
