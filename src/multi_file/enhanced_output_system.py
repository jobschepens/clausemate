...
class EnhancedOutputSystem:
    ...
    def create_enhanced_csv_output(
        ...
        enhanced_headers = [
            ...
            "chapter_file", "chapter_number", "chapter_id", "chapter_title",
            "source_file_path", "file_format", "file_size_bytes",
            
            # Global positioning
            "global_sentence_id", "global_relationship_id", 
            "chapter_boundary_marker", "cross_chapter", "cross_chapter_chain_id",
            
            # Original relationship data (existing columns)
            "sentence_id", "sentence_id_numeric", "sentence_id_prefixed", 
            "sentence_num", "first_words",
            
            # Pronoun information
            "pronoun_text", "pronoun_token_idx", "pronoun_grammatical_role",
            "pronoun_thematic_role", "pronoun_givenness", "pronoun_coref_ids",
            "pronoun_coref_base_num", "pronoun_coref_occurrence_num",
            "pronoun_coreference_link", "pronoun_coref_link_base_num",
            "pronoun_coref_link_occurrence_num", "pronoun_coreference_type",
            "pronoun_inanimate_coreference_link", "pronoun_inanimate_coref_link_base_num",
            "pronoun_inanimate_coref_link_occurrence_num", "pronoun_inanimate_coreference_type",
            "pronoun_most_recent_antecedent_text", "pronoun_most_recent_antecedent_distance",
            "pronoun_first_antecedent_text", "pronoun_first_antecedent_distance",
            "pronoun_antecedent_choice",
            
            # Clause mate information
            "num_clause_mates", "clause_mate_text", "clause_mate_coref_id",
            "clause_mate_coref_base_num", "clause_mate_coref_occurrence_num",
            "clause_mate_start_idx", "clause_mate_end_idx",
            "clause_mate_grammatical_role", "clause_mate_thematic_role",
            "clause_mate_coreference_type", "clause_mate_animacy", "clause_mate_givenness",
            
            # Enhanced analysis fields
            "narrative_position", "character_continuity_score", "discourse_coherence_score",
            "cross_chapter_strength", "chain_importance_score"
        ]
        
        # Create chapter lookup for metadata
        chapter_lookup = {meta.chapter_number: meta for meta in chapter_metadata}
        cross_chapter_lookup = {conn.chain_id: conn for conn in cross_chapter_connections}
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for rel in relationships:
                ...
                chapter_num = getattr(rel, 'chapter_number', 1)  # Default to chapter 1 for base relationships
                chapter_meta = chapter_lookup.get(chapter_num, None)
    ...
    def create_comprehensive_statistics(
        ...
        cross_chapter_rels = sum(1 for rel in relationships if getattr(rel, 'cross_chapter', False))
        
        # Calculate quality metrics
        avg_rels_per_sentence = len(relationships) / total_sentences if total_sentences > 0 else 0
        avg_chain_length = len(relationships) / total_chains if total_chains > 0 else 0
        cross_chapter_percentage = (len(cross_chapter_connections) / total_chains * 100) if total_chains > 0 else 0
    ...
    def create_chapter_boundary_report(
        ...
        for i in range(len(chapter_metadata) - 1):
            ...
            connections = [
                ...
                if (conn.from_chapter == current.chapter_number and 
                    conn.to_chapter == next_chapter.chapter_number)
            ]
            
            transition = {
                'from_chapter': current.chapter_number,
                'to_chapter': next_chapter.chapter_number,
                'sentence_gap': next_chapter.global_sentence_range[0] - current.global_sentence_range[1],
                'connections_count': len(connections),
                'connection_types': [conn.connection_type for conn in connections],
                'average_strength': sum(conn.strength for conn in connections) / len(connections) if connections else 0.0,
                'narrative_continuity_score': self._calculate_narrative_continuity(current, next_chapter, connections)
            }
        ...
        boundary_analysis['boundary_statistics'] = {
            ...
            'boundaries_with_connections': sum(1 for t in boundary_analysis['chapter_transitions'] if t['connections_count'] > 0),
            'average_connections_per_boundary': sum(t['connections_count'] for t in boundary_analysis['chapter_transitions']) / len(boundary_analysis['chapter_transitions']) if boundary_analysis['chapter_transitions'] else 0,
            'strongest_boundary': max(boundary_analysis['chapter_transitions'], key=lambda x: x['average_strength']) if boundary_analysis['chapter_transitions'] else None,
            'weakest_boundary': min(boundary_analysis['chapter_transitions'], key=lambda x: x['average_strength']) if boundary_analysis['chapter_transitions'] else None
        }
        ...
        boundary_analysis['cross_chapter_patterns'] = {
            conn_type: {
                ...
                'average_strength': sum(c.strength for c in connections) / len(connections),
                'chapter_span': {
                    'min': min(c.to_chapter - c.from_chapter for c in connections),
                    'max': max(c.to_chapter - c.from_chapter for c in connections),
                    'average': sum(c.to_chapter - c.from_chapter for c in connections) / len(connections)
                }
            }
    ...
    def _calculate_analysis_scores(
        ...
        recent_distance = getattr(relationship.pronoun, 'most_recent_antecedent_distance', None)
        if recent_distance and isinstance(recent_distance, (int, float)):
            # Closer antecedents indicate better coherence
            discourse_coherence_score = max(0.1, min(1.0, 1.0 / (1.0 + recent_distance / 10.0)))
    ...
    def _create_boundary_marker(
        ...
        # Handle both UnifiedClauseMateRelationship and base ClauseMateRelationship
        chapter_num = getattr(relationship, 'chapter_number', 1)  # Default to chapter 1 for base relationships
