...
class TSVFormatDetector:
    ...
    def analyze_file(self, file_path: str, max_sample_rows: int = 50) -> TSVFormatInfo:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                logger.info(f"Format analysis complete: {format_info.format_type} format "
    ...
    def _analyze_format(self, file_path: str, sample_rows: List[List[str]], 
                       sentence_boundaries: List[str], token_count: int,
                       sentence_count: int, total_columns: int) -> TSVFormatInfo:
        ...
        if total_columns == 15:
            ...
            column_mapping = self.standard_mapping
            issues.append("Incomplete format detected - limited coreference analysis available")
            ...
            try:
                ...
                if format_type == "incomplete":
                    ...
                    if len(sample_row) > column_mapping.coreference_link:
                        required_column_positions['coreference_link'] = column_mapping.coreference_link
                    else:
                        issues.append("Coreference link column not accessible")
                        
                    if len(sample_row) > column_mapping.coreference_type:
                        required_column_positions['coreference_type'] = column_mapping.coreference_type
                    else:
                    ...
                    if len(sample_row) > column_mapping.inanimate_coreference_link:
                        required_column_positions['inanimate_coreference_link'] = column_mapping.inanimate_coreference_link
                    else:
                        issues.append("Inanimate coreference link column not accessible")
                        
                    if len(sample_row) > column_mapping.inanimate_coreference_type:
                        required_column_positions['inanimate_coreference_type'] = column_mapping.inanimate_coreference_type
                    else:
                        issues.append("Inanimate coreference type column not accessible")
                    
                    # For incomplete formats, having basic columns is enough
                    has_required_columns = (
                        'token_id' in required_column_positions and
                        'token_text' in required_column_positions
                    )
                else:
    ...
    def _calculate_compatibility_score(self, has_required_columns: bool,
                                     format_type: str, total_columns: int,
    ...
    def compare_formats(self, format_infos: List[TSVFormatInfo]) -> Dict[str, Any]:
        ...
        sorted_formats = sorted(format_infos, key=lambda x: x.compatibility_score, reverse=True)
        ...
        compatible_files = len([info for info in format_infos if info.compatibility_score >= 0.7])
        ...
        if compatible_files == total_files:
            ...
            recommendations.append(f"{compatible_files}/{total_files} files are compatible")
            recommendations.append("Consider implementing adaptive parsing for remaining files")
        else:
            recommendations.append("No files are fully compatible - adaptive parsing required")
        
        if len(format_types) > 1:
            recommendations.append("Multiple format types detected - implement format detection")
