...
try:
    from data.models import ClauseMateRelationship, Pronoun, ClauseMate, Animacy, Givenness
except ImportError:
    ...
    with open(relationships_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            pronoun = Pronoun(
                ...
                animacy=Animacy.ANIM if row.get('pronoun_animacy') == 'anim' else Animacy.INANIM,
                givenness=Givenness.BEKANNT if row.get('pronoun_givenness') == 'bekannt' else Givenness.NEU
            )
            ...
            clause_mate = ClauseMate(
                ...
                animacy=Animacy.ANIM if row.get('clause_mate_animacy') == 'anim' else Animacy.INANIM,
                givenness=Givenness.BEKANNT if row.get('clause_mate_givenness') == 'bekannt' else Givenness.NEU
            )
...
def analyze_character_tracking(relationships, cross_chapter_chains):
    ...
    for chain_id, mentions in character_mentions.items():
        ...
        character_consistency = 1.0 - (len(set(m['grammatical_role'] for m in mentions)) / len(mentions))
        cross_chapter_continuity = len(chapters_present) / 4.0  # Normalize by total chapters
...
def analyze_narrative_flow(relationships, character_profiles):
    ...
    for chapter_num in sorted(chapters.keys()):
        ...
        total_sentences = len(set(rel.sentence_num for rel in chapter_rels))
        ...
        for i in range(4):
            ...
            segment_rels = [rel for rel in chapter_rels 
                          if start_sentence <= rel.sentence_num <= end_sentence]
            
            # Calculate metrics
            character_density = len(set(getattr(rel, 'pronoun_coref_ids', []) for rel in segment_rels)) / max(1, len(segment_rels))
            coreference_density = sum(len(getattr(rel, 'pronoun_coref_ids', [])) for rel in segment_rels) / max(1, len(segment_rels))
...
def calculate_performance_metrics(processing_stats, relationships):
    ...
    return {
        ...
        'processing_efficiency': total_relationships / total_time if total_time > 0 else 0,
        'memory_efficiency': 0.0
    }

def generate_coreference_visualization_data(relationships, character_profiles, output_dir):
    ...
    for char_id, profile in character_profiles.items():
        color = "#FF6B6B" if profile['narrative_prominence'] > 0.8 else "#4ECDC4" if profile['narrative_prominence'] > 0.5 else "#45B7D1"
...
def create_comprehensive_analysis_report(character_profiles, narrative_segments, transitions, performance_metrics, output_dir):
    ...
    character_stats = {
        "total_characters": len(character_profiles),
        "major_characters": sum(1 for p in character_profiles.values() if p['narrative_prominence'] > 0.8),
        "minor_characters": sum(1 for p in character_profiles.values() if p['narrative_prominence'] <= 0.2),
        "cross_chapter_characters": sum(1 for p in character_profiles.values() if len(p['chapters_present']) > 1),
        "average_prominence": sum(p['narrative_prominence'] for p in character_profiles.values()) / len(character_profiles) if character_profiles else 0,
        "average_continuity": sum(p['cross_chapter_continuity'] for p in character_profiles.values()) / len(character_profiles) if character_profiles else 0
    }
    
    narrative_stats = {
        "total_segments": len(narrative_segments),
        "average_character_density": sum(s['character_density'] for s in narrative_segments) / len(narrative_segments) if narrative_segments else 0,
        "average_narrative_tension": sum(s['narrative_tension'] for s in narrative_segments) / len(narrative_segments) if narrative_segments else 0,
    ...
    transition_stats = {
        ...
        "average_character_continuity": sum(t['character_continuity'] for t in transitions) / len(transitions) if transitions else 0,
        "average_narrative_coherence": sum(t['narrative_coherence'] for t in transitions) / len(transitions) if transitions else 0,
        "strong_transitions": sum(1 for t in transitions if t['narrative_coherence'] > 0.7),
...
def main():
    ...
    print(f"📊 Loaded {len(relationships)} relationships and {len(cross_chapter_chains)} cross-chapter chains")
    ...
    viz_file = generate_coreference_visualization_data(relationships, character_profiles, output_dir)
    ...
    print("\n🔍 Key Findings:")
    major_characters = sum(1 for p in character_profiles.values() if p['narrative_prominence'] > 0.8)
    cross_chapter_chars = sum(1 for p in character_profiles.values() if len(p['chapters_present']) > 1)
    avg_coherence = sum(t['narrative_coherence'] for t in transitions) / len(transitions) if transitions else 0
    ...
    print(f"   - Processing rate: {performance_metrics['relationships_per_second']:.1f} relationships/second")
