...
def test_schema_aware_parsing():
    ...
    for file_path in files_to_test:
        ...
        try:
            ...
            if format_info:
                print(f"Format: {format_info.format_type} ({format_info.total_columns} columns)")
                print(f"Compatibility: {format_info.compatibility_score:.2f}")
            
            # Show column mapping info
            if parser.current_column_mapping:
                print(f"Column mapping:")
                print(f"  Coreference link: {parser.current_column_mapping.coreference_link}")
                print(f"  Coreference type: {parser.current_column_mapping.coreference_type}")
            
            # Show annotation schema info
            if parser.current_annotation_schema:
                print(f"Annotation schema:")
                print(f"  Total columns: {parser.current_annotation_schema.total_columns}")
                ...
                if prontype_col:
                    ...
                    print(f"  No pronoun type column found")
