...
def main():
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("**Generated from Phase 2 implementation with cross-sentence antecedent analysis**\n\n")
        
        # Basic statistics
        total_rows = len(df)
        successful_antecedents = len(df[df['pronoun_most_recent_antecedent_text'] != '_'])
        success_rate = (successful_antecedents / total_rows) * 100 if total_rows > 0 else 0
        ...
        f.write("**Research Question**: Do pronouns appear at more consistent linear positions when clause mates are present?\n\n")
        
        print("=== TASK REQUIREMENT ANALYSIS ===")
        print("Checking: Do pronouns appear at more consistent positions when clause mates are present?")
    
        # Filter for successful antecedent detections with numeric distances
        valid_data = df[(df['pronoun_most_recent_antecedent_text'] != '_') & 
                        (df['pronoun_most_recent_antecedent_distance'] != '_')]
        
        if len(valid_data) > 0:
            # Convert distance to numeric
            valid_data = valid_data.copy()
            valid_data['distance_numeric'] = valid_data['pronoun_most_recent_antecedent_distance'].astype(int)
            ...
            if len(has_clause_mates) > 0 and len(few_clause_mates) > 0:
                ...
                f.write(f"**With multiple clause mates** ({len(has_clause_mates)} cases):\n")
                f.write(f"- Average pronoun-antecedent distance: {avg_dist_with_mates:.1f} tokens\n")
                f.write(f"- Distance variability (std dev): {std_with_mates:.1f}\n\n")
                
                f.write(f"**With few/no clause mates** ({len(few_clause_mates)} cases):\n")
                f.write(f"- Average pronoun-antecedent distance: {avg_dist_without_mates:.1f} tokens\n")
                f.write(f"- Distance variability (std dev): {std_without_mates:.1f}\n\n")
                
                print(f"\nClause Mate Presence Analysis:")
                print(f"With multiple clause mates (>{len(has_clause_mates)} cases):")
                print(f"  - Average pronoun-antecedent distance: {avg_dist_with_mates:.1f} tokens")
                print(f"  - Distance variability (std dev): {std_with_mates:.1f}")
                
                print(f"\nWith few/no clause mates ({len(few_clause_mates)} cases):")
                print(f"  - Average pronoun-antecedent distance: {avg_dist_without_mates:.1f} tokens")
                ...
                if avg_dist_with_mates < avg_dist_without_mates:
                    f.write("✅ **MATCHES**: Shorter average distance when clause mates present\n")
                    print("✓ MATCHES: Shorter average distance when clause mates present")
                else:
                    f.write("❌ **Different pattern**: Longer average distance when clause mates present\n")
                    print("✗ Different pattern: Longer average distance when clause mates present")
                    
                if std_with_mates < std_without_mates:
                    f.write("✅ **MATCHES**: More consistent positions when clause mates present\n\n")
                    print("✓ MATCHES: More consistent positions when clause mates present")
                else:
                    f.write("❌ **Different pattern**: More variable positions when clause mates present\n\n")
                    print("✗ Different pattern: More variable positions when clause mates present")
        ...
        high_distance = numeric_distances[numeric_distances['pronoun_most_recent_antecedent_distance'].astype(int) > 15]
        ...
        if len(high_distance) > 0:
            ...
            f.write("*Showing correct \"most recent\" antecedent selection across sentences:*\n\n")
            ...
            for idx, (i, row) in enumerate(high_distance.iterrows()):
                ...
                key = (row["pronoun_text"], row["sentence_id"], row["pronoun_most_recent_antecedent_text"])
            ...
            for idx, row in enumerate(diverse_examples):
                ...
                f.write(f"**CURRENT SENTENCE {row['sentence_id']}**: \"{current_sentence}\"\n\n")
                print(f'    CURRENT SENTENCE {row["sentence_id"]}: "{current_sentence}"')
                
                f.write(f"- **Critical pronoun**: \"{row['pronoun_text']}\" (token {row['pronoun_token_idx']})\n")
                f.write(f"- **Grammatical role**: {row['pronoun_grammatical_role']}, **Thematic role**: {row['pronoun_thematic_role']}\n")
                f.write(f"- **Clause mate**: \"{row['clause_mate_text']}\" ({row['clause_mate_grammatical_role']}, {row['clause_mate_thematic_role']})\n")
                f.write(f"- **Total clause mates in sentence**: {row['num_clause_mates']}\n\n")
                
                print(f'    Critical pronoun: "{row["pronoun_text"]}" (token {row["pronoun_token_idx"]})')
                print(f'    Grammatical role: {row["pronoun_grammatical_role"]}, Thematic role: {row["pronoun_thematic_role"]}')
                print(f'    Clause mate: "{row["clause_mate_text"]}" ({row["clause_mate_grammatical_role"]}, {row["clause_mate_thematic_role"]})')
                print(f'    Total clause mates in sentence: {row["num_clause_mates"]}')
                
                # Show most recent antecedent with its sentence context
                f.write(f"**MOST RECENT ANTECEDENT**: \"{row['pronoun_most_recent_antecedent_text']}\" (distance: {row['pronoun_most_recent_antecedent_distance']} tokens)\n")
                print(f'    MOST RECENT ANTECEDENT: "{row["pronoun_most_recent_antecedent_text"]}" (distance: {row["pronoun_most_recent_antecedent_distance"]} tokens)')
                ...
                for sent_num in range(max(1, row["sentence_id"] - 10), row["sentence_id"]):
                    sentence_text = get_sentence_text(sent_num)
                    if row["pronoun_most_recent_antecedent_text"] in sentence_text:
                        f.write(f"  - Found in **Sentence {sent_num}**: \"{sentence_text}\"\n\n")
                        print(f'      Found in Sentence {sent_num}: "{sentence_text}"')
                        antecedent_found = True
                        break
                
                if not antecedent_found:
                    f.write("  - Context: Antecedent sentence not found in recent sentences\n\n")
                    print(f'      Context: Antecedent sentence not found in recent sentences')
                
                # Show first antecedent if different
                if row["pronoun_first_antecedent_text"] != row["pronoun_most_recent_antecedent_text"]:
                    f.write(f"**FIRST ANTECEDENT**: \"{row['pronoun_first_antecedent_text']}\" (distance: {row['pronoun_first_antecedent_distance']} tokens)\n")
                    print(f'    FIRST ANTECEDENT: "{row["pronoun_first_antecedent_text"]}" (distance: {row["pronoun_first_antecedent_distance"]} tokens)')
                    ...
                    if not first_antecedent_found:
                        ...
                        print(f'      Context: First antecedent sentence not found')
                else:
                    f.write("\n")
                
                f.write(f"- **Antecedent choices available**: {row['pronoun_antecedent_choice']}\n")
                print(f'    Antecedent choices available: {row["pronoun_antecedent_choice"]}')
                
                # Show coreference information
                coref_info = []
                if row["pronoun_coreference_link"] and row["pronoun_coreference_link"] != '_':
                    coref_info.append(f"animate: {row['pronoun_coreference_link']}")
                if row["pronoun_inanimate_coreference_link"] and row["pronoun_inanimate_coreference_link"] != '_':
                    coref_info.append(f"inanimate: {row['pronoun_inanimate_coreference_link']}")
                if coref_info:
