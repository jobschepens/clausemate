...
cross_chapter_rels = df[df['cross_chapter'] == True]
if len(cross_chapter_rels) > 0:
    for i, row in cross_chapter_rels.head(5).iterrows():
        print(f'  Chapter {row["chapter_number"]}, Sentence {row["sentence_id"]}: {row["pronoun_text"]} -> {row["clause_mate_text"]}')
...
chapter_breakdown['percentage'] = (chapter_breakdown['sum'] / chapter_breakdown['count'] * 100).round(1)
