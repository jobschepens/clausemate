...
class PhaseComparator:
    ...
    def __init__(self):
        self.results = {"phase1": {}, "phase2": {}, "comparison": {}}
    ...
    def compare_csv_outputs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            comparison["phase1"] = {
                ...
                "file_size_mb": round(
                    Path(self.phase1_output).stat().st_size / (1024 * 1024), 2
                ),
            }
            ...
            comparison["phase2"] = {
                ...
                "file_size_mb": round(
                    Path(self.phase2_output).stat().st_size / (1024 * 1024), 2
                ),
            }
    ...
    def run_comparison(self) -> Dict[str, Any]:
        ...
        if self.results["phase1"].get("success") and self.results["phase2"].get(
            "success"
        ):
    ...
    def generate_report(self, results: Dict[str, Any]) -> str:
        ...
        if phase1.get("success") and phase2.get("success"):
            ...
            report.append(
                f"Difference: {time_diff:+.2f}s ({faster_phase} is {time_savings:.2f}s faster)"
            )
            ...
            report.append(
                f"File Size Difference: {size_diff:+,} bytes ({size_diff_mb:+.2f} MB)"
            )
        ...
        if "comparison" in results and "error" not in results["comparison"]:
            ...
            report.append(
                f"Phase 1: {comp['phase1']['rows']:,} rows, {comp['phase1']['columns']} columns"
            )
            report.append(
                f"Phase 2: {comp['phase2']['rows']:,} rows, {comp['phase2']['columns']} columns"
            )
            report.append(
                f"Row Difference: {comp['phase2']['rows'] - comp['phase1']['rows']:+,}"
            )
            report.append(
                f"Column Difference: {comp['phase2']['columns'] - comp['phase1']['columns']:+}"
            )
            ...
            if comp["columns"]["phase1_only_count"] > 0:
                ...
                if comp["columns"]["phase1_only_count"] > 5:
                    report.append(
                        f"   ... and {comp['columns']['phase1_only_count'] - 5} more"
                    )
            ...
            if comp["columns"]["phase2_only_count"] > 0:
                ...
                if comp["columns"]["phase2_only_count"] > 5:
                    report.append(
                        f"   ... and {comp['columns']['phase2_only_count'] - 5} more"
                    )
        ...
        if phase1.get("success") and phase2.get("success"):
            ...
            if "relationships_found" in p1_stats and "relationships_found" in p2_stats:
                rel_diff = (
                    p2_stats["relationships_found"] - p1_stats["relationships_found"]
                )
                report.append(
                    f"📈 Relationships: Phase 1: {p1_stats['relationships_found']:,}, "
            ...
            if phase2["execution_time"] < phase1["execution_time"]:
                improvement = (
                    (phase1["execution_time"] - phase2["execution_time"])
                    / phase1["execution_time"]
                ) * 100
                report.append(f"🚀 Phase 2 is {improvement:.1f}% faster")
            else:
                slowdown = (
                    (phase2["execution_time"] - phase1["execution_time"])
                    / phase1["execution_time"]
                ) * 100
