...
class DataSourceManager:
    ...
    def __init__(self, project_root: Path | None = None):
        """Initialize DataSourceManager with project root path."""
...
"""
    ...
    def _setup_local_path(self) -> bool:
        ...
        if not path_obj.exists():
            create = (
                input("Directory doesn't exist. Create it? [y/N]: ").strip().lower()
            )
            if create == "y":
                try:
...
def main():
    ...
    if choice == "1":
        ...
        env_vars = [k for k in os.environ if k.startswith("CLAUSEMATE_")]
        if env_vars:
